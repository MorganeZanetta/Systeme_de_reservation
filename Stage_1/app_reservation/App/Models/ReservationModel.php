 ...
 */
    ...
    {
        // Si l'objet rôle est déjà présent et contient un libellé, on ne fait rien
        if ($user->getRole() !== null && !empty($user->getRole()->getLib())) {
            return;
        }

        // Sinon, on récupère l'ID du rôle en base pour cet utilisateur
...
public function creerAvecRelationsEtLog(Reservation $reservation): bool 
    {
        try {
            ...
            $donneesEnregistrees = [
                ...
                'materiels' => array_map(fn($m) => ['id' => $m->getIdMateriel(), 'type' => method_exists($m, 'getTypMat') ? $m->getTypMat() : 'Non défini', 'numero' => $m->getNumMat()], $reservation->getMateriels())
            ];
...
public function modifierAvecRelationsEtLog(Reservation $reservation): bool 
{
    try {
        ...
        // 4. PRÉPARATION DES DONNÉES DE LOG
        $newData = [
            'utilisateur_id' => $reservation->getUtilisateur()->getIdUtilisateur(),
            'reservation' => [
                'motif' => $reservation->getMotif(),
        ...
        // 5. JOURNALISATION SÉCURISÉE DU RÔLE
        $user = $reservation->getUtilisateur();
        // On s'assure que le rôle est chargé avant de construire le message
        if ($user) $this->chargerRoleSiAbsent($user);
        
        $role = ($user !== null) ? $user->getRole() : null;
        $idAuteur = $_SESSION['Id_utilisateur'] ?? 'Inconnu';

        $message = (isset($_POST['bouton_modification_admin']) ? "Modification par Administrateur ID: $idAuteur. Créateur: " : "Modification par Utilisateur ID: ") . 
        ...
        public function annulerAvecRelationsEtLog(int $idRes, Utilisateur $user): bool 
        {
            try {
                ...
                // 2. PRÉPARATION DES DONNÉES POUR LE LOG (Hydratation sécurisée du rôle)
