...
class AdminController extends BaseController {
    // Propriétés privées stockant les instances des modèles pour l'interaction avec la base de données
    ...
    /**
     * Constructeur : Initialise les modèles avec la connexion PDO fournie.
     */
    ...
    /**
     * Helper : Convertit JJ/MM/AAAA vers AAAA-MM-JJ (format compatible SQL).
     */
    ...
    /**
     * Vérification de sécurité : empêche l'accès aux méthodes admin aux utilisateurs non autorisés.
     */
    ...
    /**
     * Affiche l'interface principale d'administration (gestion des utilisateurs/rôles/ports).
     */
    ...
    /**
     * Crée un nouvel utilisateur : hache le mot de passe et persiste l'objet.
     */
    public function creerUtilisateursAdmin(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Assignation du rôle et du port
            // Création du rôle
            $role = new Role();
    ...
    /**
     * Modifie un utilisateur : met à jour les données et optionnellement le mot de passe.
     */
    ...
    /**
     * Affiche l'interface spécifique à la gestion des rôles et des ports.
     */
    ...
    /**
     * Modifie le libellé d'un rôle existant.
     */
    ...
    /**
     * Supprime un rôle (précaution : vérifier contraintes clés étrangères en DB).
     */
    ...
    /**
     * Affiche l'interface de gestion des salles et matériels.
     */
    ...
    /**
     * Enregistre un nouveau matériel avec gestion optionnelle d'image.
     */
    public function creerMaterielAdmin(): void {
        ...
        if (isset($_FILES['photo_materiel']) && $_FILES['photo_materiel']['error'] === UPLOAD_ERR_OK) {
            ...
            // 4. Gestion de l'erreur : on stocke le message pour l'afficher à l'utilisateur
    ...
    public function modifierMaterielAdmin(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['Id_materiel'])) {
            try {
                // 1. Récupérer le matériel actuel pour conserver l'ancienne photo
                $idMateriel = (int)$_POST['Id_materiel'];
                $materiel = $this->materielModel->findById($idMateriel);
                ...
                $materiel->setTypMat(trim($_POST['type_materiel'] ?? ''));
                $materiel->setNumMat((int)($_POST['numero_materiel'] ?? 0));

                // 3. Gestion de la persistance de l'image
                if (isset($_FILES['photo_materiel']) && $_FILES['photo_materiel']['error'] === UPLOAD_ERR_OK) {
                    // Un nouveau fichier est envoyé : on le traite
                    $nomFichier = $_FILES['photo_materiel']['name'];
                    // (Logique de move_uploaded_file à ajouter ici si nécessaire)
                    $materiel->setPhoMat($nomFichier); 
                } else {
                    // AUCUN nouveau fichier : on ne fait rien. 
                    // La propriété $materiel->photo_materiel garde sa valeur actuelle 
                    // (celle récupérée lors du recupererMaterielParId)
                }

                // 4. Gestion de la relation Port
    ...
    /**
     * Affiche l'interface de réservation (Admin).
     */
    ...
    /**
     * Affiche la liste complète des réservations pour administration.
     */
    ...
    /**
     * Affiche le formulaire de modification d'une réservation précise.
     */
    ...
    /**
     * Crée une réservation via l'admin, inclut gestion d'erreurs (try/catch).
     */
    ...
    /**
     * Met à jour une réservation existante avec vérification d'accès.
     */
    ...
    /**
     * Récupère et affiche les logs de réservation (avec filtrage optionnel).
     */
    public function afficherTrieEtListeLogs(): void {
    $this->ensureAdmin();

    // 1. On récupère les données nécessaires pour construire les listes déroulantes.
    $utilisateurs = $this->utilisateurModel->recupererUtilisateurs();
    $salles       = $this->salleModel->voirListeSalles();
    $materiels    = $this->materielModel->voirListeMateriel();

    // 2. Logique de filtrage conditionnelle :
    // Si l'utilisateur a soumis le formulaire (cliqué sur "Valider"), on filtre.
    // Sinon, on charge la liste complète.
    if (!empty($_GET['criteres']) || !empty($_GET['barre_recherche'])) {
        // On passe les critères de filtrage ET les options de recherche (cases à cocher).
        $logs = $this->reservationLogModel->chercherParCriteres(
            $_GET['criteres'] ?? [], 
            $_GET // Contient les cases à cocher et la recherche rapide.
        );
    } else {
        // Liste par défaut (sans aucun filtre)
        $logs = $this->reservationLogModel->getAllLogsWithUsers();
    }
    
    // 3. On passe les données à la vue.
    $this->render('interfaceLogAdministrateur', [
        'logs'         => $logs,
